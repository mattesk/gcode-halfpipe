...
double lHalfpipe      = -60.00;       // 26.00 mm   length of halfpipe
double zcutHalfpipe   =  8.00;       //  4.00 mm   depth of halfpipe   (start of cuts is always at 0mm)
...
main( int argc, char **argv )
{
   ...
   //for ( int i = 40; i >= -40; i -= 1) {       //every 0.5 degrees
   for ( int i = -37; i <= 37; i += 1) {       // reverse direction  --> !smoother cut
      ...
      if ( initial ) { 
         ...
         printf("G0 Z5.000\n" );
         printf("G1 X%.3lf Y%.3lf Z%.3lf F%.3lf\n", bem_y,bem_z,bem_x+lHalfpipe, speedVertical );
         printf(")\n");
      }
      
      printf("(vhalfpipe path %02d degrees: ysin/zcos=%.3lf/%.3lf  hpcoff_yz= %.2lf %.2lf", i, ysin,zcos, hpcoff_y,hpcoff_z );
      printf("  bemx/y/z= %.2lf %.2lf %.2lf", bem_y,bem_z,bem_x );
      printf(")\n");
      
      printf("G1 X%.3lf Y%.3lf Z%.3lf F%.3lf\n", bem_y, bem_z, bem_x+lHalfpipe, speedHorizontal );
